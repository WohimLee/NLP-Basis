...
class MyRNN(nn.Module):
    def __init__(self,input_size,hidden_size,batch_first=False):
        ...
        self.U = nn.Linear(hidden_size,hidden_size) # 用于对上一时刻的 t 进行一个变换再输入到下个时刻
        self.tanh = nn.Tanh()

    def forward(self,x):
        batch_, len_, emb_n = x.shape

        t = torch.zeros((batch_,self.hidden_size),device=x.device)
        result = torch.zeros(size=(batch_,len_,self.hidden_size),device=x.device)
...
class RnnTextCls(nn.Module):
    def __init__(self):
        ...
        self.rnn = MyRNN(emb_num,rnn_hidden_num,batch_first=True)  # RNN : Linear , emb_num * 200
    ...
    def forward(self,x,lable=None):  #10 * 30  10 * 30 * 200 @ 200 * 400 = 10 * 30 * 400
        ...
        rnn_out1,rnn_out2 = self.rnn.forward(emb)
